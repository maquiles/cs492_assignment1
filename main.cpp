...
void *producer(void* i){
    ...
    while (true){
        ...
        if(id < product_num){
            ...
            uint64_t t = tv.tv_sec*(uint64_t)10000000 + tv.tv_usec; //get the current timestamp
        ...
        if(consumed < product_num){
            ...
            if(p != nullptr){ //handle the product if life longer than quantum
                if(p->life >= quantum){
                    ...
                    gettimeofday(&tv, nullptr);
                    curr_time = tv.tv_sec*(uint64_t)10000000 + tv.tv_usec;
                    av_w += curr_time - p->ts;
...
void *consumer_FCFS(void* i){
    ...
    while(true){
        ...
        if(consumed++ < product_num){
            ...
            gettimeofday(&tv, nullptr);
            curr_time = tv.tv_sec*(uint64_t)10000000 + tv.tv_usec; //update the current time
            av_w += curr_time - p->ts;
...
int main(int argc, char *argv[]){
    ...
    gettimeofday(&tv, nullptr);
    uint64_t s_time = tv.tv_sec*(uint64_t)10000000 + tv.tv_usec;
    ...
    uint64_t e_time = tv.tv_sec*(uint64_t)10000000 + tv.tv_usec; 
